        ...
        <div key={`row-${i}`} className="grid md:grid-cols-1 lg:grid-cols-2 gap-8">
          {postsInRow.map((post, postIndex) => (
            <Link key={post.slug} href={`/posts/${post.slug}`} passHref>
              <article className="bg-white overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 group cursor-pointer">
                ...
                <div className="flex items-center justify-between mb-8">
                  <div>
                    <h2 className="text-3xl font-bold text-black">
                      {isFiltered ? 'Filtered Results' : 'All News'}
                    </h2>
