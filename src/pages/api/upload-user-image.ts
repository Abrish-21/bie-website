...
export default async function uploadUserImageHandler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method !== 'POST') {
  ...
  try {
    ...
    const cloudinaryUploadResult = await cloudinary.uploader.upload(tempPath, {
      folder: 'bie-website-user-profiles', // Use a new folder for user profiles
      resource_type: 'image',
    });
    ...
    console.error('[DEBUG_CATCH_ERROR] User image upload failed:', error);
